...
def generate():
    ...
    # --- OPTIMIZED PDF GENERATION: Hardcoded Text Positions ---
    # This replaces the CSV file reading loop for a slight performance boost.
    c.setFont("Helvetica", 11)

    # Position from CSV: x=2.0836, y=4.8789
    c.drawString(2.0836 * 72, (11 - (4.8789 + 0.1584)) * 72, values['Transaction number'])
    
    # Position from CSV: x=2.0836, y=5.2539
    c.drawString(2.0836 * 72, (11 - (5.2539 + 0.1584)) * 72, values['Authorization code'])
    
    # Position from CSV: x=2.0836, y=5.6289
    c.drawString(2.0836 * 72, (11 - (5.6289 + 0.1584)) * 72, values['Response code'])
    
    # Position from CSV: x=2.0836, y=6.0039
    c.drawString(2.0836 * 72, (11 - (6.0039 + 0.1584)) * 72, values['Space number'])
    
    # Position from CSV: x=2.0836, y=6.3789
    c.drawString(2.0836 * 72, (11 - (6.3789 + 0.1584)) * 72, values['Start of session'])
    
    # Position from CSV: x=2.0836, y=6.7539
    c.drawString(2.0836 * 72, (11 - (6.7539 + 0.1584)) * 72, values['End of session'])
    
    # Position from CSV: x=2.0836, y=4.5039
    c.drawString(2.0836 * 72, (11 - (4.5039 + 0.1584)) * 72, values['Top date line'])
    
    # Position from CSV: x=2.0836, y=6.6827
    c.drawString(2.0836 * 72, (11 - (6.6827 + 0.1584)) * 72, values['Reference number'])
    
    # Add Transaction Date (new)
    tx_x = 1.9515 * 72
    tx_y = (11 - (4.8789 + 0.1584)) * 72
    c.drawString(tx_x, tx_y, transaction_datetime)
    # --- END OPTIMIZED PDF GENERATION ---
...
def scan_ticket():
    ...
    if file:
        try:
            ...
            # 3. Capture the date and time for the 'au' part of the date range OR 'Date de signification'
            ...
            print(f"Extracted Data: Date={extracted_date}, Time={extracted_time}")
